...
def split_text(text):
    ...
    while start < len(token_ids):
        ...
        part_text = TOKENIZER.decode(token_ids[start:end]).strip()
...
def split_table(table_text):
    ...
    if count_tokens(table_text) <= CHUNK_SIZE:
        return [table_text.strip()]

    lines = [line for line in table_text.splitlines() if line.strip()]

    if len(lines) < 2:
        return [item["text"] for item in split_text(table_text)]
    ...
    for row in data_rows:
        ...
        if (
            count_tokens(candidate_text) > CHUNK_SIZE
            and len(current_lines) > len(header_lines)
        ):
        ...
        if count_tokens(current_text) > CHUNK_SIZE:
            fallback_parts = split_text(current_text)
            table_parts.extend(part["text"] for part in fallback_parts)
            current_lines = header_lines.copy()

    if len(current_lines) > len(header_lines):
        table_parts.append("\n".join(current_lines).strip())

    return [part for part in table_parts if part.strip()]
...
def chunk_paper(paper):
    ...
    sections_path = SECTIONS_DIR / paper_id / "sections.json"
    ...
    if not sections:
        raise ValueError(f"No sections found for {paper_id}.")
    ...
    for section in sections:
        ...
        for block in blocks:
            ...
            segments = separate_content(block_text)

            for segment in segments:
                if segment["type"] == "table":
                    table_parts = split_table(segment["text"])
                    ...
                    text_parts = split_text(segment["text"])
        ...
        retrieval_enabled = section_name != "references"
        total_chunks = len(prepared_chunks)

        for index, prepared in enumerate(prepared_chunks, start=1):
            chunk_id = f"{section['section_id']}_chunk_{index:03d}"
            page_number = prepared["page_number"]

            embedding_text = (
                f"Paper: {paper['title']}\n"
                f"Paper short name: {paper['short_name']}\n"
                f"Section: {section['display_name']}\n"
                f"Content type: {prepared['chunk_type']}\n"
                f"Page: {page_number}\n\n"
                f"{prepared['text']}"
            )
            ...
            paper_chunks.append(
                {
                    ...
                    "include_in_default_retrieval": retrieval_enabled,
                    "text": prepared["text"],
...
def process_all_papers():
    ...
    for paper in papers:
        ...
        print()
        print(f"Chunking {paper_id}: {paper['short_name']}")

        chunks = chunk_paper(paper)

        if not chunks:
            raise ValueError(f"No chunks were created for {paper_id}.")
        ...
        text_count = sum(chunk["chunk_type"] == "text" for chunk in chunks)
        table_count = sum(chunk["chunk_type"] == "table" for chunk in chunks)
        enabled_count = sum(chunk["retrieval_enabled"] for chunk in chunks)
        reference_count = sum(chunk["section"] == "references" for chunk in chunks)

        print(f"  Chunks: {len(chunks)}")
        print(f"  Text chunks: {text_count}")
        print(f"  Table chunks: {table_count}")
        print(f"  Retrieval enabled: {enabled_count}")
        print(f"  Reference chunks disabled: {reference_count}")
        ...
        summary.append(
            {
                ...
                "retrieval_enabled_count": enabled_count,
            }
        )
    ...
    multi_page_chunks = sum(
        chunk["page_start"] != chunk["page_end"] for chunk in all_chunks
    )

    save_json(CHUNKS_DIR / "all_chunks.json", all_chunks)
    save_json(CHUNKS_DIR / "summary.json", summary)
    ...
    print(
        ...
        f"{sum(chunk['retrieval_enabled'] for chunk in all_chunks)}"
    )
    print(
        "Table chunks: "
        f"{sum(chunk['chunk_type'] == 'table' for chunk in all_chunks)}"
    )
    print(f"Multi-page chunks: {multi_page_chunks}")
    print(
        "Largest chunk: "
        f"{max(chunk['token_count'] for chunk in all_chunks)} cl100k tokens"
    )
