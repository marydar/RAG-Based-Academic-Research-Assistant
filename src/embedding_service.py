"""Generate reliable multilingual E5 embeddings.

The model is loaded in this order:

1. data/models/multilingual-e5-small
2. data/model_cache_e5_v2
3. Default Hugging Face cache

No internet connection is required if the model already exists
...
def test_model_candidate(
    ...
    if np.array_equal(
        ...
        raise RuntimeError(
            "Tokenizer produced identical "
            "token IDs for different questions."
        )
    ...
    if np.allclose(
        ...
        raise RuntimeError(
            "Model produced identical "
            "embeddings for different questions."
        )

    if cosine_similarity >= 0.9999:
        raise RuntimeError(
            "Model produced almost identical "
            "embeddings.\n"
            f"Cosine similarity: "
...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        print(
            "Startup embedding test passed."
        )

        print(
            "Probe cosine similarity: "
            f"{probe_similarity:.8f}"
        )

        print(
            "Probe maximum difference: "
            f"{probe_difference:.8f}"
        )
    ...
    def _load_working_model(self):
        ...
        raise RuntimeError(
            "No valid multilingual-e5-small "
    ...
    def check_lengths(
        ...
        if oversized:
            ...
            raise ValueError(
                f"{len(oversized)} inputs "
                f"exceed the "
                f"{MAX_LENGTH}-token limit."
            )
